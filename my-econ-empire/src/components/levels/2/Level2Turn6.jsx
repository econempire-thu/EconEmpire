...
import Lvl2Turn6ProductionPanel from "./Lvl2Turn6ProductionPanel.jsx";
import Lvl2Turn6TradePanel from "./Lvl2Turn6TradePanel.jsx";
import BuildingOptions from "./BuildingOptions.jsx";

function Level2Turn6({
  onResultsOpen,
  score,
  villageLetter,
  levelTurnKey,
  ...
  const [hasMeatCamp, setHasMeatCamp] = useState(false);
  const [hasBerriesHut, setHasBerriesHut] = useState(false);
  const [hasTimberLodge, setHasTimberLodge] = useState(false);
  ...
  const loadData = async (user) => {
    ...
    const meatCamp = await getUserField("meatCamp", false, user);
    const berriesHut = await getUserField("berriesHut", false, user);
    const timberLodge = await getUserField("timberLodge", false, user);
    setHasMeatCamp(meatCamp);
    setHasBerriesHut(berriesHut);
    setHasTimberLodge(timberLodge);
  };
  ...
  useEffect(() => {
    setPossibleBuildings({
      "Meat Camp": hasMeatCamp,
      "Berries Hut": hasBerriesHut,
      "Timber Lodge": hasTimberLodge,
    });
  }, [hasMeatCamp, hasBerriesHut, hasTimberLodge]);
  ...
  const getAngryCount = (sumOfMBR) => {
    return sumOfMBR < 2 * totalVillagers
      ? Math.max(-100, 2 * totalVillagers - sumOfMBR)
      : 0;
  };
        ...
        <>
          {!productionSet && !productionExists && (
            <>
              ...
              <Lvl2Turn6ProductionPanel
                ...
                hasMeatCamp={hasMeatCamp}
                hasBerriesHut={hasBerriesHut}
                hasTimberLodge={hasTimberLodge}
              />
          ...
          {productionExists && !tradeEnded && (
            <>
              ...
              {Object.values(possibleBuildings).includes(false) && (
                <BuildingOptions
                  refreshData={loadData}
                  user={user}
                  timber={availableResources.timber}
